...
(()=>{
 ...
 function proceed(){if(requested||failed)return;requested=true;root.dataset.state='loading';fadeAt=performance.now()+650;maybeReveal();}
 root.addEventListener('click',e=>{if(e.target!==retry)proceed();});retry.addEventListener('click',()=>location.reload());
 window.addEventListener('keydown',e=>{if(!window.pirateStartup.active)return;if(['Enter','Space'].includes(e.code)){e.preventDefault();e.stopImmediatePropagation();if(!e.repeat)(failed?retry:button).click();}else if(e.code.startsWith('Arrow')){e.preventDefault();e.stopImmediatePropagation();(failed?retry:button).classList.add('is-selected');}},true);
