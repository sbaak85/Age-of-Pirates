...
const volumeMat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,side:THREE.BackSide,
 ...
 fragmentShader:explosionBFragment});
...
const haloMaterial=new THREE.MeshBasicMaterial({color:0xffa323,transparent:true,opacity:0,blending:THREE.AdditiveBlending,depthWrite:false});
const halo=new THREE.Mesh(new THREE.TorusGeometry(1,.018,6,128),haloMaterial);halo.rotation.x=Math.PI/2;scene.add(halo);
...
function animate(now){requestAnimationFrame(animate);const dt=Math.min((now-last)/1000,.05);last=now;
 ...
 const t=Math.max(0,elapsed-.25);volume.visible=t>0&&t<4.2;volumeMat.uniforms.uTime.value=t;volumeMat.uniforms.uEye.value.copy(camera.position);
