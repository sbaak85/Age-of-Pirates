...
const {ship}=createShip();ship.scale.setScalar(.72);ship.position.set(-7,.27,-3);scene.add(ship);batchStatic(ship);
const camera=new THREE.PerspectiveCamera(48,1,.1,200);camera.position.set(10,10,14);
...
const variants=[createWaterMist()],panels=[...document.querySelectorAll('.view')];variants.forEach(v=>scene.add(v.group));
...
$('freeze').onclick=()=>{elapsed=.75;paused=true;sync();};
...
function frame(now){requestAnimationFrame(frame);const dt=document.hidden?0:Math.min(.05,(now-last)/1000)*(slow?.35:1);last=now;
 ...
 controls.update();if(!paused){elapsed+=dt;if(elapsed>=3.2)replay();}
 const t=Math.max(0,elapsed-.3);variants.forEach(v=>v.update(t));ball.visible=elapsed<.3;ball.position.set((.3-elapsed)*9,Math.max(.08,5*(1-elapsed/.3)),0);
 ...
 panels.forEach((panel,i)=>{const rect=panel.getBoundingClientRect();variants.forEach((v,j)=>{v.group.visible=j===i&&t>0&&t<2.4;});
   camera.aspect=rect.width/rect.height;camera.updateProjectionMatrix();variants[i].prepare(camera);
 ...
 });renderer.setScissorTest(false);$('status').textContent=`${paused?'已暫停':'同步播放'} · ${t.toFixed(2)} s`;
}
